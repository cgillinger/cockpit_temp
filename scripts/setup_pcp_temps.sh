...
RETENTION_DAYS="${RETENTION_DAYS:-365}"        # Keep archives this many days
MAX_SIZE_GB="${MAX_SIZE_GB:-5}"                # Max total archive size in GB
...
# Configure pmlogger_daily retention via /etc/sysconfig/pmlogger or /etc/default/pmlogger
PMLOGGER_SYSCONFIG=""
if [[ -d /etc/sysconfig ]]; then
    PMLOGGER_SYSCONFIG="/etc/sysconfig/pmlogger"
elif [[ -d /etc/default ]]; then
    PMLOGGER_SYSCONFIG="/etc/default/pmlogger"
fi

if [[ -n "$PMLOGGER_SYSCONFIG" ]]; then
    info "Skriver retentionskonfiguration till $PMLOGGER_SYSCONFIG"
    # -k N: keep N days; -x 0: compress immediately; -E: expunge after retention
    cat > "$PMLOGGER_SYSCONFIG" <<SYSEOF
# Cockpit-temps PCP archive retention settings
# Generated by setup_pcp_temps.sh
PMLOGGER_DAILY_PARAMS="-E -k ${RETENTION_DAYS} -x 0"
SYSEOF
    ok "pmlogger_daily retention: ${RETENTION_DAYS} dagar, komprimering aktiv."
else
...
# Create disk-budget enforcement script
BUDGET_SCRIPT="/etc/cron.daily/pcp-archive-budget"
...
# Set the configured max size
if [[ -n "$PMLOGGER_SYSCONFIG" ]]; then
    echo "PCP_MAX_SIZE_GB=${MAX_SIZE_GB}" >> "$PMLOGGER_SYSCONFIG"
fi

ok "Disk-budget-skript installerat (max ${MAX_SIZE_GB} GB)."
echo ""
